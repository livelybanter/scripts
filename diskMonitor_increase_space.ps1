...
# if there was any row at all, send the email, increase disk space.
if ($regex.IsMatch($regexsubject)) {
                        Get-HardDisk -vm $computers | where {$_.Name -eq VolumeName} | Set-HardDisk -CapacityGB 20 -ResizeGuestPartition -Confirm:$false -WhatIf
                        #| where {$_.Name -eq $tableFragment} #| Set-HardDisk -CapacityGB "1" -ResizeGuestPartition -Confirm:$false
